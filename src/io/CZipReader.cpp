...
IReadFile* createLimitedReadFile(const c8* fileName, IReadFile* alreadyOpenedFile, s32 areaSize);

//-----------------------------------------------------------------------------

CZipReader::CZipReader(
	IReadFile* file, bool ignoreCase, bool ignorePaths
	) : 
File(file), IgnoreCase(ignoreCase), IgnorePaths(ignorePaths)
{
#if MY_DEBUG_MODE  
	IUnknown::setClassName("CZipReader");
#endif
    ...
    case 0: // no compression
        {
            ...
            return createLimitedReadFile(FileList[index].simpleFileName.c_str(), File, FileList[index].header.DataDescriptor.UncompressedSize);
        }
    case 8:
        {
            #ifdef _IRR_COMPILE_WITH_ZLIB_
            ...
            if (!pBuf)
            {
                LOGGER.log("Not enough memory for decompressing", FileList[index].simpleFileName.c_str(), io::ELL_ERROR);
                return 0;
            }
            ...
            if (err != Z_OK)
            {
                LOGGER.log("Error decompressing", FileList[index].simpleFileName.c_str(), io::ELL_ERROR);
                delete [] (c8*)pBuf;
                return 0;
            }
            else
        ...
        break;
    default:
        LOGGER.log("file has unsupported compression method.", FileList[index].simpleFileName.c_str(), io::ELL_ERROR);
        return 0;
    };
}
...
s32 CZipReader::findFile(const c8* simpleFilename)
{
    ...
    //если не нашли то попробуем еще раз
    if (res == -1)
    {
        for (u32 i=0; i<FileList.size(); ++i)
        {   core::stringc name;
            SZipFileEntry entr=FileList[i];
            if (IgnorePaths)
                name = FileList[i].simpleFileName;
            else
                name = FileList[i].zipFileName;
            if (name == entry.simpleFileName)
            {   res=i;
                break;
            }
        }
    ...
    if (res == -1)
    {
        for (u32 i=0; i<FileList.size(); ++i)
            ...
            {
                LOGGER.log("File in archive but not found.", entry.simpleFileName.c_str(), io::ELL_ERROR);
                break;
            }
    }
    #endif
