...
{
#if MY_DEBUG_MODE 
	IUnknown::setClassName("CImageLoaderPSD");
#endif
}
...
//! returns true if the file maybe is able to be loaded by this class
bool CImageLoaderPSD::isALoadableFileFormat(io::IReadFile* file)
{
    if (!file)
        return false;
...
IImage* CImageLoaderPSD::loadImage(io::IReadFile* file)
{
    delete [] imageData;
    imageData = 0;
    ...
    if (header.version != 1)
    {
        LOGGER.log("Unsupported PSD file version", file->getFileName(), io::ELL_ERROR);
        return 0;
    }

    if (header.mode != 3 || header.depth != 8)
    {
        LOGGER.log("Unsupported PSD color mode or depth. ", file->getFileName(), io::ELL_ERROR);
        return 0;
    }
    ...
    if (!file->seek(l, true))
    {
        LOGGER.log("Error seeking file pos to image resources. ", file->getFileName(), io::ELL_ERROR);
        return 0;
    }
    ...
    if (!file->seek(l, true))
    {
        LOGGER.log("Error seeking file pos to layer and mask. ", file->getFileName(), io::ELL_ERROR);
        return 0;
    }
    ...
    if (!file->seek(l, true))
    {
        LOGGER.log("Error seeking file pos to image data section. ", file->getFileName(), io::ELL_ERROR);
        return 0;
    }
    ...
    {
        LOGGER.log("Unsupported psd compression mode. ", file->getFileName(), io::ELL_ERROR);
        return 0;
    }
    ...
    {
        // create surface
        image = new CImage(ECF_A8R8G8B8, core::dimension2d<s32>(header.width, header.height), imageData);
    }
...
bool CImageLoaderPSD::readRLEImageData(io::IReadFile* file)
{
    /*  If the compression code is 1, the image data
        starts with the byte counts for all the scan lines in the channel
        (LayerBottom LayerTop), with each count stored as a twoЦ
        ...
        the TIFF standard.
        If the LayerТs Size, and therefore the data, is odd, a pad byte will
    ...
    */
    ...
    for (u32 y=0; y<header.height * header.channels; ++y)
    {
        if (!file->read(&rleCount[y], sizeof(u16)))
        {
            delete [] tmpData;
            delete [] rleCount;
            LOGGER.log("Error reading rle rows ", file->getFileName(), io::ELL_ERROR);
            return false;
        }
...
IImage* CImageLoaderPSD::loadImage(const c8* fileName)
{   
