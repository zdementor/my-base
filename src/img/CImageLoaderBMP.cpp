...
//! constructor
CImageLoaderBMP::CImageLoaderBMP() : BmpData(0), PaletteData(0)
{
#if MY_DEBUG_MODE  
	IUnknown::setClassName("CImageLoaderBMP");
#endif
}
...
//! returns true if the file maybe is able to be loaded by this class
bool CImageLoaderBMP::isALoadableFileFormat(io::IReadFile* file)
{
    u16 headerID;
...
IImage* CImageLoaderBMP::loadImage(io::IReadFile* file)
{   SBMPHeader header;
    s32 pitch = 0;
    ...
    {
		LOGGER.logErr("Incorrect header of the BMP file.");
        return 0;
    }
    ...
    {
        LOGGER.log("Compressed BMPs are currently not supported.", io::ELL_ERROR);
        return 0;
    }
    ...
    {
        // okay, wir dьrfen raten wie gross die bitmap ist.
    ...
    case 1:
        ...
        CColorConverter::convert1BitTo16BitFlipMirror(
			BmpData, (s16*)image->getData(), header.Width, header.Height, pitch);
        break;
    case 4:
        image = new CImage(ECF_A1R5G5B5, core::dimension2d<s32>(header.Width, header.Height));
        CColorConverter::convert4BitTo16BitFlipMirror(
			BmpData, (s16*)image->getData(), header.Width, header.Height, pitch, PaletteData);
        break;
    case 8:
        image = new CImage(ECF_A1R5G5B5, core::dimension2d<s32>(header.Width, header.Height));
        CColorConverter::convert8BitTo16BitFlipMirror(
			BmpData, (s16*)image->getData(), header.Width, header.Height, pitch, PaletteData);
        break;
    case 16:
        break;
    case 24:
        ...
        CColorConverter::convert24BitTo24BitFlipMirrorColorShuffle(
			BmpData, (c8*)image->getData(), header.Width, header.Height, pitch);
        break;
    case 32:
		image = new CImage(ECF_A8R8G8B8, core::dimension2d<s32>(header.Width, header.Height));
        CColorConverter::convert32BitTo32Bit(
			(s32*)BmpData, (s32*)image->getData(), header.Width, header.Height, 0, true);
        break;
    };
...
IImage* CImageLoaderBMP::loadImage(const c8* fileName)
{   
