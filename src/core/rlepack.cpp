...
{   
    ...
    while (1)
    {
        if (get_byte(&ch, in_buf, in_buf_size, out_buf, out_buf_size) == EOD)  // read next byte into ch 
           break;
    ...
    )
{
     ...
     while (get_byte(&ch, in_buf, in_buf_size, out_buf, out_buf_size) != EOD)
     {
        if (ch != tmpbuf[1])
        {
